...
char po_title[14][128], sc_title[12][128], wa_title[10][128], ri_title[11][128];
...
{
    short i, j, maxlen, n;
    char descs[ROGUE_LINES][256];
    chtype saved_screen[ROGUE_LINES][ROGUE_COLUMNS];
    short row, col;
    struct id *id;
	...
	for (j = col; j < ROGUE_COLUMNS; j++) {
	    saved_screen[row][j - col] = mvinch_rogue(row, j);
	}
	move(row, col);
	clrtoeol();
	...
	clrtoeol();
	for (j = col; j < ROGUE_COLUMNS; j++) {
	    addch_rogue(saved_screen[row][j - col]);
	}
    }
