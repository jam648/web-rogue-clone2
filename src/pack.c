...
pack_letter(char *prompt, unsigned short mask)
{
    ...
    short row, col;
    char *p;
    chtype saved[ROGUE_LINES][ROGUE_COLUMNS];
    extern char descs[ROGUE_LINES][256];
    extern int utf8width(const char *);
    extern void get_desc(object *, char *, boolean);
    ...
    /* Draw list of matching items on right side of screen */
    obj = rogue.pack.next_object;
    i = 0;
    maxlen = 10;
	...
	for (j = col; j < ROGUE_COLUMNS; j++) {
	    saved[row][j - col] = mvinch_rogue(row + 1, j);
	}
	move(row + 1, col);
	clrtoeol();
	mvaddstr_rogue(row + 1, col, descs[row]);
    }
    ...
    for (;;) {
	ch = rgetchar();
	if (ch == CANCEL || (ch >= 'a' && ch <= 'z') || ch == LIST || ch == '?' || ch == '!' || ch == ')' || ch == ']' || ch == '/' || ch == '=' || ch == ':' || ch == ',') {
	    break;
	}
	sound_bell();
    }
	...
	clrtoeol();
	for (j = col; j < ROGUE_COLUMNS; j++) {
	    addch_rogue(saved[row][j - col]);
	}
    }
    check_message();
    refresh();

    if (ch == LIST || ch == '?' || ch == '!' || ch == ')' || ch == ']' || ch == '/' || ch == '=' || ch == ':' || ch == ',') {
	inventory(&rogue.pack, mask);
	return pack_letter(prompt, mask);
    }
    return ch;
}
