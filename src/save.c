...
save_into_file(char *sfile)
{
	...
	message("セーブに失敗しました。", 0);
    } else {
	extern void wasm_notify_save(void);
	wasm_notify_save();
    }
...
restore(char *fname)
{
    ...
    if (!login_name[0] || (unsigned char)login_name[0] < 32) {
        strcpy(login_name, "RODNEY");
    }
    for (int k = 0; login_name[k]; k++) {
        if ((unsigned char)login_name[k] < 32) {
            login_name[k] = '\0';
            break;
        }
    }
...
{
    short i, j;
    RogueCell cell_buf[ROGUE_COLUMNS];

    if (rw) {    // 書き込み時実行
	for (i = 0; i < ROGUE_LINES; i++) {
	    r_write(fp, (char *) dungeon[i], (ROGUE_COLUMNS * sizeof(dungeon[0][0])));
	    for (j = 0; j < ROGUE_COLUMNS; j++) {
		cell_buf[j] = rogue_screen[i][j];
	    }
	    r_write(fp, (char *) cell_buf, sizeof(cell_buf));
	}
	return;
    } else {    // 読み込み時実行
	for (i = 0; i < ROGUE_LINES; i++) {
	    r_read(fp, (char *) dungeon[i], (ROGUE_COLUMNS * sizeof(dungeon[0][0])));
	    r_read(fp, (char *) cell_buf, sizeof(cell_buf));

	    if ( i < ROGUE_LINES - 1 ) {
		for (j = 0; j < ROGUE_COLUMNS; j++) {
		    wasm_screen_set_char(i, j, cell_buf[j].ch, cell_buf[j].color, cell_buf[j].attr);
		}
	    } else {
		print_stats(STAT_ALL);
	    }
...
write_string(char *s, FILE *fp)
{
    ...
    if (!s) s = "";
    n = (short)(strlen(s) + 1);
    if (n > (short)sizeof(buf)) n = (short)sizeof(buf);
