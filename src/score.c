...
/*	static char xpos[] = {35,34,33,32,31,30,30,30,30,30,30,30,29,21};*/
	...
	p = os1[other];
	q = os2[other];
	snprintf(death_reason, sizeof(death_reason), "%s%s", p, q);
    } else {
	...
	center(13, buf2);
	strcpy(buf, mesg[180]);
	lznum(buf + strlen(buf), rogue.gold, 0);
	center(10, buf);
	*buf = 0;
    ...
    message("", 0);
    wasm_notify_game_over((int)rogue.gold, cur_level, max_level, rogue.gold, death_reason);
...
center(short row, char *buf)
{
    ...
    margin = ((ROGUE_COLUMNS - utf8width(buf)) / 2);
