...
char descs[ROGUE_LINES][256];	/* multi-purpose screen saver */
static chtype saved_screen_area[ROGUE_LINES][ROGUE_COLUMNS];
...
nextpage:
	...
	for (j = col; j < ROGUE_COLUMNS; j++) {
	    saved_screen_area[row][j - col] = mvinch_rogue(row, j);
	}
	move(row, col);
	clrtoeol();
	...
	clrtoeol();
	for (j = col; j < ROGUE_COLUMNS; j++) {
	    addch_rogue(saved_screen_area[row][j - col]);
	}
    }
...
make_scroll_titles(void)
{
	...
	for (j = 0; j < sylls; j++) {
	    ...
	    if (strlen(id_scrolls[i].title) + strlen(syllables[s]) + strlen("」") >= MAX_TITLE_LENGTH) {
		break;
	    }
